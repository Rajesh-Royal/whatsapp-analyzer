...
class GetWhatsappChatInsights {
  ...
  getUniqueUsernames(): string[] {
    ...
    const allUsernames: string[] = this.chatDatabase
      ...
      .filter((author) => author !== null || author !== 'system') as string[];
  ...
  generateEmojiData() {
    ...
    this.chatDatabase.forEach((message) => {
      totalMessagesByUser[message.author as string] =
        (totalMessagesByUser[message.author as string] || 0) + 1;
      ...
      if (emojisInMessage) {
        emojisInMessage.forEach((emoji) => {
          ...
          const existingEmoji = userEmojiData.emojiUsage.find(
            (e) => e.emoji === emoji,
          );
          if (existingEmoji) {
            existingEmoji.value++;
    ...
    Object.keys(emojiData).forEach((username) => {
      ...
      userEmojiData.emojiStat.emojiPerText =
        userEmojiData.emojiStat.totalEmojis / totalMessages;
      userEmojiData.emojiStat.totalUniqueEmojis =
        userEmojiData.emojiUsage.length;
    });

    // Calculate emoji statistics for All users
    Object.values(emojiData).forEach((userEmojiData) => {
      emojiData.All.emojiStat.totalEmojis +=
        userEmojiData.emojiStat.totalEmojis;
      emojiData.All.emojiUsage.push(...userEmojiData.emojiUsage);
    });
    emojiData.All.emojiStat.emojiPerText =
      emojiData.All.emojiStat.totalEmojis / this.chatDatabase.length;
    emojiData.All.emojiStat.totalUniqueEmojis = [
      ...new Set(emojiData.All.emojiUsage.map((emoji) => emoji.emoji)),
    ].length;
  ...
  chatSummary(): {
    ...
    const totalWords = this.chatDatabase.reduce(
  ...
  generateWordCloud() {
    ...
    Object.keys(wordCloudData).forEach((username) => {
      ...
      wordCloudData[username].wordStat.mostUsedWord =
        sortedWords[0]?.text || "";
      wordCloudData[username].wordStat.leastUsedWord =
        sortedWords[sortedWords.length - 1]?.text || "";
    });
    ...
    const sortedAllWords = Object.keys(allWordUsage).sort(
      (a, b) => allWordUsage[b] - allWordUsage[a],
    );
    ...
    wordCloudData.All = {
      ...
      wordUsage: sortedAllWords.map((word) => ({
        text: word,
        value: allWordUsage[word],
      })),
    };
  ...
  generateRadarMap(): RadarMapData {
    ...
    const radarMapUsage: RadarMapUsage = Object.entries(hourStats).map(
      ([time, count]) => ({ count, time: parseInt(time) }),
    );

    // Calculate average texts per hour, least active hour, and most active hour
    const totalMessages = Object.values(hourStats).reduce(
      (total, count) => total + count,
      0,
    );
    const averageTextsPerHour = totalMessages / 24;
    const leastActiveHour = Object.entries(hourStats).reduce(
      (prev, [hour, count]) => (count < prev[1] ? [hour, count] : prev),
      ["0", Infinity],
    )[0];
    const mostActiveHour = Object.entries(hourStats).reduce(
      (prev, [hour, count]) => (count > prev[1] ? [hour, count] : prev),
      ["0", 0],
    )[0];
    ...
    const users = this.chatInsightsVars.uniqueUserNames.filter(
      (author) => author !== "system",
    );
    users.forEach((user) => {
      const userMessages = this.chatDatabase.filter(
      ...
      const radarMapUsageUser: RadarMapUsage = Object.entries(
        hourStatsUser,
      ).map(([time, count]) => ({ count, time: parseInt(time) }));

      // Calculate average texts per hour, least active hour, and most active hour for the current user
      const totalMessagesUser = Object.values(hourStatsUser).reduce(
        (total, count) => total + count,
        0,
      );
      const averageTextsPerHourUser = totalMessagesUser / 24;
      const leastActiveHourUser = Object.entries(hourStatsUser).reduce(
        (prev, [hour, count]) => (count < prev[1] ? [hour, count] : prev),
        ["0", Infinity],
      )[0];
      const mostActiveHourUser = Object.entries(hourStatsUser).reduce(
        (prev, [hour, count]) => (count > prev[1] ? [hour, count] : prev),
        ["0", 0],
      )[0];
  ...
  generateChatTimeline(): ChatTimeline {
    ...
    this.chatDatabase.forEach((message) => {
      ...
      if (!this.isSystemUser(author) && author !== null) {
        ...
        chatTimeline[author] = chatTimeline[author] || {
          timelineStat: { mostActiveDate: "", value: 0 },
          timelineUsage: [],
        };
        const existingUsage = chatTimeline[author].timelineUsage.find(
          (usage) => usage.date === formattedDate,
        );
        if (existingUsage) {
          existingUsage.count++;
        } else {
          chatTimeline[author].timelineUsage.push({
            date: formattedDate,
            count: 1,
          });
        }
        ...
        // Update 'All' timeline
        const existingAllUsage = allTimelineUsage.find(
          (usage) => usage.date === formattedDate,
        );
        if (existingAllUsage) {
    ...
    const mostActiveDateAll = allTimelineUsage.reduce((prev, curr) =>
      curr.count > prev.count ? curr : prev,
    ).date;
    const mostActiveCountAll =
      allTimelineUsage.find((usage) => usage.date === mostActiveDateAll)
        ?.count || 0;
    chatTimeline.All = {
      timelineStat: {
        mostActiveDate: mostActiveDateAll,
        value: mostActiveCountAll,
      },
      timelineUsage: allTimelineUsage,
    };
    ...
    Object.keys(chatTimeline).forEach((author) => {
      if (author !== "All") {
        ...
        const mostActiveDate = timelineUsage.reduce((prev, curr) =>
          curr.count > prev.count ? curr : prev,
        ).date;
        const mostActiveCount =
          timelineUsage.find((usage) => usage.date === mostActiveDate)?.count ||
          0;
        chatTimeline[author].timelineStat = {
          mostActiveDate,
  ...
  generateUserSpecificInformation(): UserSpecificInfo {
    ...
    this.chatInsightsVars.uniqueUserNames
      .filter((username) => !this.isSystemUser(username))
      .forEach((username) => {
        const userMessages = this.chatDatabase.filter(
          (message) =>
            message.author === username && !this.isSystemUser(username),
        );
        ...
        const totalWords = userMessages.reduce(
          (sum, message) => sum + message.message.split(/\s+/).length,
          0,
        );
        ...
        const totalMedia = userMessages.filter((message) =>
          message.message.includes("<Media omitted>"),
        ).length;
        const totalLinks = userMessages.filter((message) =>
          message.message.includes("http"),
        ).length;
        const messageDates = userMessages.map((message) =>
        ...
        const averageWordsPerMessage = totalMessages
