...
const ChatInsightDayWise = () => {
  ...
  const chatInsightBasedOnDays = useChatInsightStore((store) => store.stats.basedOnDays);
    ...
    <Section title='Breakdown of your chats, day-wise'>
      ...
      <div className="grid gap-4 lg:grid-cols-3 md:grid-cols-2">
        <ChatStatsBox
          ...
          stats={
            (chatInsightBasedOnDays[selectedUser as 'All'][1] as { averageTexts: number; leastActiveDay: string; mostActiveDay: string; })
        ...
        <ChatStatsBox
          ...
          stats={(chatInsightBasedOnDays[selectedUser as 'All'][1] as { averageTexts: number; leastActiveDay: string; mostActiveDay: string; })
            .leastActiveDay}
...
export const DayWiseBarChart: React.FC<{ selectedOption: string }> = ({ selectedOption = "All" }) => {
  const chatInsightBasedOnDays = useChatInsightStore((store) => store.stats.basedOnDays)

  const days = sortedDays(chatInsightBasedOnDays[selectedOption as 'All'][0] as { DAY: string; MESSAGE: number; }[])

  return (
    <ResponsiveContainer height={400}>
      <BarChart
        height={400}
        data={chatInsightBasedOnDays[selectedOption as 'All'][0] as { DAY: string; MESSAGE: number; }[]}
        layout="vertical"
      >
        <XAxis type="number" />
