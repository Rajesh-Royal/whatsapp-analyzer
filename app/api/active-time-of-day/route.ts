...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: "Fetched active time analytics data successfully",
      status: 200,
      data: [result],
      count: null,
    });
    ...
    return NextApiErrorHandler(
      error,
      "Failed to fetch active time analytics data",
    );
  }
}
...
const getMostActiveTime = async (author: string | null) => {
  ...
  const mostActiveTimeByEachAuthorEachHour = messages.reduce(
    (acc, message) => {
  ...
  for (const author in mostActiveTimeByEachAuthorEachHour) {
    mostActiveTimeByEachAuthor[author] = mostActiveTimeByEachAuthorEachHour[
      author
